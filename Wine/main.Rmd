...
df_raw_1 <- as_tibble(read_csv("wine_new.csv"))
df_raw_2 <- as_tibble(read_csv("wine.csv"))
...
```{r}
data.frame(taster_name = c('Carrie Dykes', 'Fiona Adams', 'Michael Schachner', 'Alexander Peartree', 'Jeff Jenssen', 'Lauren Buzzeo', 'Sean P. Sullivan', 'Jim Gordon', 'Mike DeSimone', 'Susan Kostrzewa', 'Christina Pickard', 'Roger Voss', 'Joe Czerwinski', 'Kerin O\'Keefe'),
           sex = c('f', 'f', 'm', 'm', 'm' ,'f', 'm', 'm', 'm', 'f', 'f', 'm', 'm',)
           )
```
...
```{r}
points_by_taster <- df %>% group_by(taster_name) %>%
  summarize(points_mean = mean(points, na.rm = TRUE),
            points_sd = sd(points, na.rm = TRUE),
            count = n()) %>% 
  drop_na(taster_name)

p <- ggplot(points_by_taster, aes(x = points_mean, y = reorder(taster_name, points_mean)))
p + geom_pointrange(aes(xmin = points_mean - points_sd,
                        xmax = points_mean + points_sd), fatten = 0.75) + 
  scale_x_continuous(limits = c(82,96), expand = c(0,0.5))
...
ggsave('1.jpg', width = 10, height = 6, scale = 0.75)
...
p_out_3 <- p3 + geom_density_ridges_gradient(scale = 1.75,
                                 size = 0.25,
                                 rel_min_height = 0.01,
